...
三条通道,依次降级:
  1. OneBound 1688 商品 API(配了 ONEBOUND_KEY/ONEBOUND_SECRET 时):真图 + 真实批发价
  2. Claude 联网搜索(web_search/web_fetch 服务端工具):找真实商品页
  3. 纸面题:从价格库随机组合规格,无图纯文字——保证每天必有一题
...
def main():
    ...
    quiz = (try_onebound(client, digest, recent_ids)
